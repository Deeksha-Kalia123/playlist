...
# Mysql client for connecting with mysql database.
...
#Redis client for connecting with Redis database.
REDIS_CLIENT = Redis.new(:url => DEFAULT_REDIS_URL, size: 5, network_timeout: 3)

# Sidekiq client for sending job to redis .
Sidekiq.configure_client{|config| config.redis = { url: SIDEKIQ_REDIS_URL, namespace: 'workers' }}
# Sidekiq server for receiving job from redis.
Sidekiq.configure_server{|config| config.redis = { url: SIDEKIQ_REDIS_URL, namespace: 'workers' }}


class DataUpdate

  def self.find_accounts_and_websites
    account_ids = []
    ...
    website_ids.each do |website_id|
      all_urls = []
      batch_size = 100
      MYSQL_CLIENT.query("Select id,url,hash_url from webtracking.pages#{account_id} where website_id = #{website_id} LIMIT #{batch_size}").each do |row|
        all_urls << row
      end
      puts ">>#{all_urls}"
    end
  ...
  def process_page_urls(website_id)
